...
const Registrar = () => {
  ...
  const handleCreateUser = handleSubmit(async (data, e) => {
    ...
    const newUser = {
      username: data.fullName.replaceAll(" ", "").slice(0, 10).toLowerCase(),
      email: data.email,
    ...
    <Container>
      ...
      <Form onSubmit={handleCreateUser}>
        <Label>
          ...
          <Input {...register("fullName")} placeholder="fulano_123" />
          <Error>{formState.errors.fullName?.message?.toString()}</Error>
        </Label>
        <Label>
